...
def banner():
 ...
 ######   #     #  #     #  #    #  ######   #######   #####    #####   ##### 
         ...
         {Y}☠ CALL BOMBER — BY darkboss1 Ak47 ☠{N}
""")
...
def disclaimer():
    ...
    slow(f"{G}Author: darkboss1 (Ak47)", 0.03)
    slow(f"{G}Team  : Hunter revulation X open 🩸", 0.03)
...
def about():
    clear()
    banner()
    slow(f"{G}Tool Name : darkboss1 Ak47 Call Bomber", 0.02)
    slow(f"{G}Version   : 2.1", 0.02)
    slow(f"{G}Author    : darkboss1 Ak47", 0.02)
    slow(f"{G}Team      : Hunter revulation X open 🩸", 0.02)
    slow(f"{Y}API Used  : darkboss1-Security Expert{N}", 0.02)
